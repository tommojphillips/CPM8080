...
static int file_searching_flag;
static struct _finddata_t c_file;
static intptr_t h_file;

static void snth() {
	/* Search for nth */
	int find_result = 0;
	while ((find_result = _findnext(h_file, &c_file)) == 0) {
		if (c_file.attrib & _A_SUBDIR) continue;
		if (fcb_is_valid_fn(c_file.name) == 0) break;
	}

	if (find_result == 0) {
		fcb_fill_info((FILE_CONTROL_BLOCK*)&cpm.memory[cpm.dma_address], c_file.name, c_file.size);
		file_searching_flag = 1;
		cpm.cpu.registers[REG_A] = 0;
	}
	else {
		_findclose(h_file);
		file_searching_flag = 0;
...
static void bdos_f_sfirst() {
	...
	if (strchr(cpm.fn_buffer, '*') == NULL && strchr(cpm.fn_buffer, '?') == NULL) {
		...
		h_file = _findfirst(cpm.fn_buffer, &c_file);
		if (h_file == -1L) {
			cpm.cpu.registers[REG_A] = 0xFF;
		}
		else {
			if ((c_file.attrib & _A_SUBDIR) == 0) {
				if (fcb_is_valid_fn(c_file.name) == 0) {
					file_searching_flag = 1;
					fcb_fill_info(dma_fcb, c_file.name, c_file.size);
					cpm.cpu.registers[REG_A] = 0;
				}
			}
			else {
